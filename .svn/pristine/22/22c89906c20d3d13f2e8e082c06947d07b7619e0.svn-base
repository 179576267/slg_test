...
//                    ch.pipeline().addLast(new IdleStateHandler(30, 0, 30, TimeUnit.SECONDS));//心跳30秒检测一次
            ...
            ChannelFuture f = serverBootstrap.bind(ConfigFactory.MAIN_SERVER_TCP_PORT).sync();
            if(f.isSuccess())
            {
                PrintUtils.info(logger, "TCP Server Start Success------Port---【" + ConfigFactory.MAIN_SERVER_TCP_PORT + "】");
                f.channel().closeFuture().sync();
            }
            else
            {
                PrintUtils.info(logger, "TCP Server Start Failed------Port---【" + ConfigFactory.MAIN_SERVER_TCP_PORT + "】");
            }
        } catch (Exception e) {
            e.printStackTrace();
            stop();
    ...
    {
        PrintUtils.info(logger, "Close TCPServer..................");
